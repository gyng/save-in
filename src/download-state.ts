...
export const hydrateDownloads = (state: DownloadsState, storage: StorageReader | undefined) => {
  if (!state.hydration) {
    state.hydration = getSession(storage, SESSION_KEY).then((res) => {
      const stored: Record<string, DownloadRecord> = res[SESSION_KEY] || {};
...
export const mergeDownload = (
  ...
  return updateSession(sessionWrites, storage, SESSION_KEY, (stored: any) =>
    capDownloads(
      Object.assign({}, stored, { [downloadId]: merged }) as Record<string, DownloadRecord>,
    ),
  );
};
