...
type PathSegment = ReturnType<typeof Path.PathSegment.String>;
...
interface VariableApi {
  withUrl<T>(str: string, cb: (url: URL) => T): T | string;
  padDateComponent(num: number): string;
  ...
  MONTH_NAMES: string[];
  toISOWeek(d: Date): number;
  toDelimited(str: string | null | undefined, delimiter: string): string;
  toTld(hostname: string | null | undefined): string;
  toRootDomain(hostname: string | undefined): string | undefined;
  MIME_EXTENSIONS: Record<string, string>;
  mimeToExtension(mime: string | null | undefined): string;
  resolveHead(opts: DownloadInfo): Promise<HeadResult>;
  resolveMime(opts: DownloadInfo): Promise<string>;
  resolveContent(opts: DownloadInfo): NonNullable<DownloadInfo["contentPromise"]>;
  transformers: Record<string, Transformer>;
  applyVariables<P extends object>(path: P, opts?: DownloadInfo): Promise<P>;
}
...
export const Variable: VariableApi = {
  ...
  applyVariables: async <P extends object>(path: P, opts: DownloadInfo = {}) => {
        ...
        (await Promise.all(
          variablePath.buf.map((t, i, arr) => {
            if (t.type === PATH_SEGMENT_TYPES.VARIABLE) {
              const transformer = Variable.transformers[t.val];
              if (transformer) {
