...
// Exported so downstream marshalling (downloads/wire-state.ts) can reuse the
// exact field list and numeric guards instead of redeclaring the contract.
export const WIRE_INFO_STRING_FIELDS = [
  "url",
  "sourceUrl",
  "pageUrl",
  "frameUrl",
...
// Exported so downloads/wire-state.ts shares these numeric guards instead of
// redeclaring the wire contract's validation rules.
export const isRoutingCounter = (value: unknown): value is number =>
  typeof value === "number" && Number.isSafeInteger(value) && value >= 0;
export const isBrowserTabId = (value: unknown): value is number =>
  typeof value === "number" && Number.isSafeInteger(value) && value >= 0;
