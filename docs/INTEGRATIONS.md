...
External validation is isolated from Save In's browser state: a trace uses only the sample fields supplied by the caller and never falls back to the active tab. Requests are bounded to 32,768 characters for each editable grammar, 4,096 characters per ordinary sample field, and 8,192 characters per sample URL. Unsafe or conservatively high-complexity regular expressions return `BAD_REQUEST`; bursts above 20 validation requests per 10 seconds per sender return `RATE_LIMITED`.
