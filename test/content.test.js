...
describe("findSource", () => {
  ...
  test("finds media directly under the cursor", () => {
    ...
    expect(ClickToSave.findSource(event(img), false)).toBe(
      "http://x.test/pic.png"
    );
  });
  ...
  test("finds media below an overlay via elementsFromPoint", () => {
    document.body.innerHTML =
      '<div id="overlay"></div><img id="i" src="http://x.test/pic.png">';
    const overlay = document.getElementById("overlay");
    const img = document.getElementById("i");
    document.elementsFromPoint = jest.fn(() => [overlay, img]);

    expect(ClickToSave.findSource(event(overlay), false)).toBe(
      "http://x.test/pic.png"
    );
  });
  ...
  test("falls back to the enclosing link when no media is found (#226)", () => {
    document.body.innerHTML =
      '<a href="/files/doc.pdf"><span id="s">PDF</span></a>';
    const span = document.getElementById("s");

    expect(ClickToSave.findSource(event(span), true)).toBe(
      "http://localhost/files/doc.pdf"
    );
  });
  ...
  test("media wins over an enclosing link", () => {
    document.body.innerHTML =
      '<a href="/page.html"><img id="i" src="http://x.test/pic.png"></a>';
    const img = document.getElementById("i");

    expect(ClickToSave.findSource(event(img), true)).toBe(
      "http://x.test/pic.png"
    );
  });
  ...
  test("ignores non-downloadable link schemes", () => {
    document.body.innerHTML =
      '<a href="javascript:void(0)"><span id="s">x</span></a>';
    const span = document.getElementById("s");
  ...
  test("returns undefined for plain elements", () => {
    ...
    expect(
      ClickToSave.findSource(event(document.getElementById("p")), true)
    ).toBeUndefined();
  });
});
...
describe("input helpers", () => {
  test("isKeyboardComboActive requires every combo key to be down", () => {
    ...
    expect(ClickToSave.isKeyboardComboActive([18, 17], { 18: true })).toBe(
      false
    );
