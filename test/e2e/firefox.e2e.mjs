...
const requestOptionsReload = async () => {
  await session.reloadExtensionPage("src/options/options.html");
};
...
const startPageServer = async () => {
  const server = http.createServer((req, res) => {
    if (req.url === "/pic.png") {
      ...
      res.end(
        '<html><body><a id="image-link" href="/opened"><img id="img" src="/pic.png" width="50" height="50"></a></body></html>',
      );
    }
  });
...
test("click-to-save rejects synthetic input and handles trusted single, double, and long clicks", async () => {
  ...
  try {
    ...
    const longMouseId = "save-in-long-click";
    const longContext = await session.bidi.findContext(targetUrl);
    const performLongActions = (/** @type {Array<Record<string, unknown>>} */ actions) =>
    ...
    expect(
      parseJson(
        await session.evaluateInTab(targetUrl, "JSON.stringify(window.__saveInLongClickEvents)"),
        arrayOf(decodeString),
      ),
    ).toEqual(["mousedown", "mouseup", "click"]);
    expect(await session.evaluateInTab(targetUrl, "location.hash")).toBe("#opened");
    await session.evaluateInTab(
      targetUrl,
    ...
    expect(await session.evaluateInTab(targetUrl, "location.hash")).toBe("");
...
test("middle and right click gestures on a link save without the native action", async () => {
  // Measured for this case (Firefox 152, BiDi input): a synthesized middle
  // click on a link opens it in a new tab unless content cancels `auxclick`,
  // and a synthesized right click delivers a cancelable `contextmenu` — the
  // defaults the matched gesture must cancel alongside its own mousedown.
  // The page registers its observers at parse time, before the content
  // script, so it still sees suppressed events; defaultPrevented is read in
  // a queued task because the extension's capture listener runs later.
  const server = http.createServer((req, res) => {
    if (req.url === "/pic.png") {
      res.writeHead(200, { "Content-Type": "image/png" });
      res.end(PNG);
  ...
  const clickWithButton = async (
    /** @type {number} */ button,
    /** @type {{x: number, y: number}} */ point,
  ) => {
    const context = await session.bidi.findContext(targetUrl);
    await session.bidi.send("input.performActions", {
      context,
      actions: [
        {
          type: "pointer",
  ...
  try {
    ...
    const created = await control.tabs.create({ url: pageUrl });
    const fixtureTab = await control.tabs.wait(
      created.id === undefined ? { urlIncludes: targetUrl } : { id: created.id },
    );
